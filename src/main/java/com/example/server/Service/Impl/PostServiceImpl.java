...
public class PostServiceImpl implements PostService {
    ...
    public ApiResponse createPost(PostRequestDto postRequestDto) {
        ...
        User user = userRepository.findById(UUID.fromString(postRequestDto.getId()))
                .orElseThrow(() -> new EntityNotFoundException("User profile not found"));

        Post post = new Post();
        post.setContent(postRequestDto.getContent());
        post.setCreatedAt(now.toOffsetDateTime());
        post.setSharedPost(null);
        post.setVisibility(postRequestDto.getVisibility() != null ? postRequestDto.getVisibility() : "all");
        if (Objects.equals(postRequestDto.getPostStatus(), "news & events")) {
            post.setPostStatus("pending");
            User userRole = userRepository.findByRole("ADMIN")
                    .orElseThrow(() -> new EntityNotFoundException("There is no role ADMIN"));
            notificationService.notifyAdmin(userRole, post, user);
        }
...
//     fetch all posts (R)
    ...
    @Override
    @Transactional(readOnly = true)
    public ApiResponse fetchPostTag(final String postType) {
        List<Post> posts = postRepository.findByVisibility(postType);
        List<Map<String, Object>> responseDataList = new ArrayList<>();
        ...
        for (Post post : posts) {
            ...
            responseData.put("created_at", post.getCreatedAt());
            responseData.put("visibility", post.getVisibility());
            responseData.put("postStatus", post.getPostStatus());
            responseData.put("uploaded_at", post.getUploadedAt());
            responseData.put("shared_post_id", post.getSharedPost());
        ...
        return ApiResponse.builder()
                ...
                .code(HttpStatus.OK.value())
                .data(responseDataList)
                .message("Post with visibility tag are fetched successfully")
                .build();
    }
    ...
    public ApiResponse sharedPost(PostRequestDto postRequestDto) {
        ZonedDateTime now = ZonedDateTime.now();
        User user = userRepository.findById(UUID.fromString(postRequestDto.getId())).orElseThrow(() -> new EntityNotFoundException("User profile not found"));
        ...
        sharedPost.setCreatedAt(now.toOffsetDateTime());
    ...
    public ApiResponse updatedPosts(PostRequestDto postRequestDto) {
        ...
        Post post = postRepository.findByIdAndUserId(postRequestDto.getPostId(), UUID.fromString(postRequestDto.getId()))
        ...
        post.setUploadedAt(now.toOffsetDateTime());
    ...
    // delete posts (D)
    @Override
    public ApiResponse deletePosts(final String userId, final Long postId) {
        Post post = postRepository.findByIdAndUserId(postId, UUID.fromString(userId)).orElse(null);
        if (post == null){
            return ApiResponse.builder()
                    .success(0)
                    .code(HttpStatus.CREATED.value())
